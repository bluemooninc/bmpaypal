...
function __autoload($class_name) {
	...
	try {
		if( ! include $filename ){
			throw new Exception("faild by autoload (ToT)/");
		}
	} catch(Exception $e) {
...
/*
 * Boot ActionFrame by 'action=Foo'
 */
$actionName = xoops_getrequest('action') ? xoops_getrequest('action') : null;
$actionFrame = false;
if ($actionName){
	require_once _MY_MODULE_PATH . "/class/ActionFrame.class.php";
	$actionFrame = $mydirname ."_ActionFrame";
	$moduleRunner = new $actionFrame(TRUE);
	$moduleRunner->setExecuteView(false);
 ...
 */
if($actionFrame){
	$controllerClass::executeForward($root->mController);
}
$root = XCube_Root::getSingleton();
